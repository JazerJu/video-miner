...
模型设置：用于配置字幕断句和字幕翻译使用的大语言模型 (LLM)。当前面板把断句 LLM (Split LLM) 与翻译 LLM (Translate LLM) 分开保存，因此两类任务可以使用不同供应商、模型和代理策略。

## 断句 LLM (Split LLM)

| 设置 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| 启用 LLM 断句 | 开关 | 开启 | 开启后使用 LLM 对 ASR 输出的字级时间戳进行断句优化。关闭后 ASR 直接断句。 |
...
| 启用单句翻译<sup>1</sup>  | 复选框 | 关闭 | 面向本地 vLLM 部署的翻译模型，例如 Hunyuan-MT-7B，不要求 JSON 格式输出。 |

![]()

> <sup>1</sup> ： 本地翻译模型由于幻觉问题，无法在保持JSON输出的同时，准确输出多个句子的翻译结果，并且翻译不完整的句子时会自动"补全上下文"。因此以句号为单位将句子拼接并翻译。缺点是翻译后，还原的断句位置没有保证。
...
> <sup>2</sup> ：兼容所有OpenAI格式的请求
> <sup>3</sup> ：cerebras对注册用户有一定的免费API额度。

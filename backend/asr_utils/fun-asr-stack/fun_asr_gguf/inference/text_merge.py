...
def merge_transcription_results(
    ...
    for i, res in enumerate(results):
        ...
        # 辅助函数：将字符索引映射回 segment 索引
        def _char_to_seg_idx(segs, char_idx):
            """将文本中的字符位置映射回 segment 列表中的索引"""
            cum = 0
        ...
        if match.size >= 2: # 至少匹配上 2 个字符
            # match.a 是 buffer_overlap_text 中的字符索引，需要映射回 segment 索引
            # match.b 是 curr_overlap_text 中的字符索引，同理
            
            # a. 截断 buffer：找到匹配起点对应的 segment
            buf_seg_idx = _char_to_seg_idx(buffer_overlap_segs, match.a)
            if 0 <= buf_seg_idx < len(buffer_overlap_segs):
                target_seg = buffer_overlap_segs[buf_seg_idx]
                
                # 找到 target_seg 在 full_segments 中的索引 (从后往前找)
                global_idx = -1
            ...
            cur_seg_idx = _char_to_seg_idx(curr_overlap_segs, match.b)
            if 0 <= cur_seg_idx < len(curr_overlap_segs):
                match_seg = curr_overlap_segs[cur_seg_idx]
                match_idx_in_curr = -1

...
class VideoDataView(JsonView):
    ...
    def category_json(self, cat):
        ...
        return {
            "id": cat.id,
            "name": cat.name,
            "collections": [self.collection_json(c) for c in cat.collections.all()],
    ...
    def get(self, request):
        ...
        # 过滤掉属于隐藏分类的未归档项目
        uncated_cols = Collection.objects.filter(
            category__isnull=True
        ).prefetch_related("videos")
        uncated_vids_query = Video.objects.filter(category__isnull=True)
        if hidden_category_ids:
            # 排除属于隐藏分类的视频（如果视频的分类在隐藏列表中）
            uncated_vids = uncated_vids_query.exclude(
                category_id__in=hidden_category_ids
            )
        else:
...
class LastVideoDataView(JsonView):
    ...
    def video_json(self, v):
        ...
        return {
            ...
            "url": v.url,
            "length": v.video_length,
            "last_modified": calc_diff_time(
                v.last_modified if v.last_modified else timezone.now()
            ),
    ...
    def get(self, request):
        ...
        try:
            ...
            if hidden_category_ids:
                recent_videos_query = recent_videos_query.exclude(
                    category_id__in=hidden_category_ids
                )

            recent_videos = (
                recent_videos_query.order_by("-last_modified", "-id")[
                    # 二级排序确保一致性
                    :50
                ]  # 最多50个视频
            )
...
class VideoActionView(View):
    ...
    def upload(self, request, video_id):
        ...
        if "video_file" in request.FILES:
            ...
            return JsonResponse(
                {"error": "No video or audio file provided"}, status=400
            )
        ...
        try:
            ...
            temp_file_path = os.path.join(
                save_dir, f"temp_{md5_value}{original_extension}"
            )
            ...
            if not is_audio:
                try:
                    thumbnail_filename = self._auto_generate_thumbnail(
                        final_file_path, md5_value
                    )
            ...
            try:
                ...
                print(
                    f"[Auto-generating waveform] Starting waveform generation for: {filename}"
                )
                waveform_result = get_waveform_for_file(filename)
                if waveform_result:
                    print(
                        f"[Auto-generating waveform] Successfully generated waveform for: {filename}"
                    )
                else:
                    print(
                        f"[Auto-generating waveform] Failed to generate waveform for: {filename}"
                    )
            except Exception as e:
                print(
                    f"[Auto-generating waveform] Error generating waveform for {filename}: {str(e)}"
                )
            ...
            return JsonResponse(
                {
                    ...
                    "conversion_type": "FLAC→M4A"
                    if needs_conversion
                    else ("H.265→AV1" if conversion_performed else "None"),
                    "is_audio": is_audio,
                    "video_length": formatted_duration,
    ...
    def rename(self, request, video_id):
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"Database error: {str(e)}"}, status=500
            )

        return JsonResponse(
            {"success": True, "message": "Video renamed successfully"}, status=200
        )
    ...
    def delete(self, request, video_id):
        ...
        # 准备包含删除摘要的响应
        message = (
            f"Video deleted successfully. Removed {len(deleted_files)} related files."
        )
        if errors:
            ...
            for saved_dir, file_type in media_dirs:
                ...
                for file_name in saved_files:
                    ...
                    try:
                        ...
                        print(
                            f"[ERROR] Failed to delete thumbnail for {file_name}: {e}"
                        )
                else:
    ...
    def watch(self, request, subpath):
        ...
        return render(
            request, "video.html", {"video_path": video_path, "video_id": video.id}
        )
    ...
    def update_thumbnail(self, request, video_id):
        ...
        url = os.path.splitext(os.path.basename(video.url))[
            0
        ]  # 获取文件名，不包含扩展名
        ...
        return JsonResponse(
            {"success": True, "thumbnail_url": thumbnail_filename}, status=200
        )
    ...
    def move_category(self, request, video_id):
        ...
        if cat_id is None and not cat_name:  # 同时为空 → 语义错误
            return JsonResponse(
                {
                    "success": False,
                    "error": "categoryId 或 categoryName 必须至少提供一个",
                },
                status=400,
            )
        ...
        if video.collection:
            ...
            # 3. 合集没有分类但要移动到特定分类
            if (
                target_category is None and video.collection.category_id is not None
            ) or (
                target_category is not None
                and video.collection.category_id != target_category.id
            ):
    ...
    def load_chapters(self, request, video_id):
        ...
        try:
            ...
            return JsonResponse({"success": True, "chapters": chapters}, status=200)
    ...
    def get_screenshot(self, request, video_id):
        ...
        try:
            ...
            if not os.path.exists(video_path):
                return JsonResponse(
                    {"success": False, "error": "Video file not found"}, status=404
                )
            ...
            if "chapters" in data:
                ...
                for chapter in chapters:
                    ...
                    # 使用FFmpeg生成截图
                    success = self._generate_screenshot_ffmpeg(
                        video_path, timestamp, screenshot_path
                    )
                    ...
                    if success:
                        ...
                        print(
                            f"Failed to generate screenshot for chapter {chapter_id} at {timestamp}s"
                        )
                        results.append(
                            {
                                "chapterId": chapter_id,
                ...
                return JsonResponse(
                    {"success": True, "screenshots": results}, status=200
                )
    ...
    def _auto_generate_thumbnail(self, video_path, md5_value):
        try:
            ...
            extract_time = (
                max(0.5, duration * 0.1) if duration and duration > 0 else 0.5
            )
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=30, stderr=subprocess.PIPE
            )
    ...
    def load_notes(self, request, video_id):
        ...
        try:
            ...
            return JsonResponse({"success": True, "notes": notes}, status=200)
    ...
    def upload_note_image(self, request, video_id):
        ...
        try:
            ...
            if not file.name:
                return JsonResponse(
                    {"success": False, "error": "No selected file"}, status=400
                )
            ...
            return JsonResponse(
                {"success": False, "error": f"Image upload failed: {str(e)}"},
                status=500,
            )
    ...
    def upload_attachment(self, request, video_id):
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Attachment upload failed: {str(e)}"},
                status=500,
            )
    ...
    def serve_attachment(self, request, video_id):
        ...
        try:
            ...
            if not attachment_id:
                return JsonResponse(
                    {"success": False, "error": "attachment_id parameter is required"},
                    status=400,
                )

            # Get attachment and verify it belongs to this video
            attachment = get_object_or_404(
                VideoAttachment, id=attachment_id, video=video, is_active=True
            )
            ...
            if not os.path.exists(full_path):
                return JsonResponse(
                    {"success": False, "error": "Attachment file not found"}, status=404
                )
            ...
            return JsonResponse(
                {"success": False, "error": "Attachment not found"}, status=404
            )
        except Exception as e:
            return JsonResponse({"success": False, "error": str(e)}, status=500)
    ...
    def get_video_collection(self, request, video_id):
        ...
        try:
            ...
            if not video.collection:
                return JsonResponse(
                    {"success": False, "message": "Video is not in any collection"},
                    status=404,
                )
            ...
            return JsonResponse(
                {
                    ...
                    "thumbnail": f"img/{collection.thumbnail_url}"
                    if collection.thumbnail_url
                    else "",
                    "type": "collection",
                    "last_modified": calc_diff_time(
                        collection.last_modified or timezone.now()
                    ),
                },
                status=200,
            )
    ...
    def extract_audio(self, request, video_id):
        ...
        try:
            ...
            if is_audio_file(video.url):
                return JsonResponse(
                    {"success": False, "error": "This is already an audio file"},
                    status=400,
                )
            ...
            if result.returncode == 0 and os.path.exists(audio_path):
                ...
                print(
                    f"Audio extraction successful: {audio_filename} ({audio_size} bytes)"
                )
            ...
            return JsonResponse(
                {"success": False, "error": "Audio extraction timed out"}, status=500
            )
        except Exception as e:
            print(f"Audio extraction error: {str(e)}")
            return JsonResponse(
                {"success": False, "error": f"Audio extraction failed: {str(e)}"},
                status=500,
            )
    ...
    def generate_waveform_peaks(self, request, video_id):
        ...
        try:
            ...
            if not is_audio_file(video.url):
                print(
                    f"[Waveform Generation] Video file detected: {video.url}, checking for audio file..."
                )
                ...
                for fmt in audio_formats:
                    ...
                    if os.path.exists(test_path):
                        ...
                        print(
                            f"[Waveform Generation] Found existing audio file: {existing_audio_filename}"
                        )
                        break
                ...
                if existing_audio_filename:
                    ...
                    print(
                        f"[Waveform Generation] No audio file found, extracting audio from video..."
                    )
                    ...
                    try:
                        ...
                        if hasattr(extraction_response, "content"):
                            response_data = json.loads(
                                extraction_response.content.decode("utf-8")
                            )
                        else:
                        ...
                        if response_data.get("success"):
                            ...
                            if audio_filename:
                                ...
                                print(
                                    f"[Waveform Generation] Audio extraction successful: {audio_filename}"
                                )
                            else:
                                return JsonResponse(
                                    {
                            ...
                            if extraction_response.status_code == 409:
                                ...
                                if audio_path:
                                    ...
                                    audio_filename = os.path.basename(
                                        audio_path.replace("/media/saved_audio/", "")
                                    )
                                    target_filename = audio_filename
                                    print(
                                        f"[Waveform Generation] Using existing audio file: {audio_filename}"
                                    )
                                else:
                                    # 回退：根据视频文件名推断音频文件名
                                    ...
                                    if os.path.exists(test_path):
                                        target_filename = audio_filename
                                        print(
                                            f"[Waveform Generation] Found audio file by inference: {audio_filename}"
                                        )
                                    else:
                                        return JsonResponse(
                                            {
            ...
            try:
                ...
                print(
                    f"[Waveform Generation] Generating waveform for filename: {target_filename}"
                )
                waveform_data = get_waveform_for_file(target_filename)

                if waveform_data:
                    # 获取生成的文件信息
                    waveform_exists_after, new_waveform_path = has_waveform_peaks(
                        target_filename
                    )
                    file_size = (
                        os.path.getsize(new_waveform_path)
                        if waveform_exists_after
                        else 0
                    )
                    ...
                    return JsonResponse(
                        {
                            ...
                            "samples_per_second": waveform_data.get(
    ...
    def get_dimensions(self, request, video_id):
        ...
        try:
            ...
            if is_audio_file(video.url):
                return JsonResponse(
                    {"success": False, "error": "Cannot get dimensions for audio file"},
                    status=400,
                )
            ...
            if result.returncode == 0:
                ...
                if "streams" in data and len(data["streams"]) > 0:
                    ...
                    # 没有视频流
                    return JsonResponse(
                        {"success": False, "error": "No video stream found in file"},
                        status=400,
                    )
            else:
                return JsonResponse(
            ...
            return JsonResponse(
                {"success": False, "error": "Video analysis timed out"}, status=500
            )
        except Exception as e:
            return JsonResponse({"success": False, "error": str(e)}, status=500)
...
class BatchVideoActionView(View):
    ...
    def move_videos_to_collection(self, request, data):
        ...
        if not video_ids or not isinstance(video_ids, list):
            return JsonResponse(
                {"success": False, "error": "Invalid videoIds"}, status=400
            )
        ...
        if not videos.exists():
            return JsonResponse(
                {"success": False, "error": "No valid videos found"}, status=404
            )
    ...
    def batch_delete_videos(self, request, data):
        ...
        for video in videos:
            try:
                ...
                if video_errors:
                    errors.extend(
                        [f"Video {video.name}: {err}" for err in video_errors]
                    )
    ...
    def concat_videos(self, request, data):
        ...
        if not video_ids or not isinstance(video_ids, list) or len(video_ids) < 2:
            return JsonResponse(
                {
                    "success": False,
                    "error": "At least 2 videos required for concatenation",
                },
                status=400,
            )
        ...
        for video_id in video_ids:
            try:
                ...
                return JsonResponse(
                    {"success": False, "error": f"Video with ID {video_id} not found"},
                    status=404,
                )
        ...
        for i, video in enumerate(videos):
            print(video.url)
            if not video.url or not os.path.exists(
                os.path.join(settings.MEDIA_ROOT, "saved_video", video.url)
            ):
                validation_errors.append(
                    f"Video {i + 1} ({video.name}): file not found"
                )
        ...
        if validation_errors:
            return JsonResponse(
                {
                    "success": False,
                    "error": "Video file validation failed",
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Concatenation failed: {str(e)}"},
                status=500,
            )
    ...
    def _check_video_compatibility(self, videos):
        ...
        if len(subtitle_languages) > 1:
            warnings.append(
                f"Videos have different subtitle languages: {', '.join(subtitle_languages)}"
            )
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(file_list_path, "w", encoding="utf-8") as f:
                    for video in videos:
                        video_path = os.path.join(
                            settings.MEDIA_ROOT, "saved_video", video.url
                        )
                ...
                temp_output_path = os.path.join(
                    settings.MEDIA_ROOT, "saved_video", output_name
                )
                ...
                final_output_path = os.path.join(
                    settings.MEDIA_ROOT, "saved_video", final_output_name
                )
                ...
                self._create_concatenated_video_record(
                    videos, output_name, video_durations
                )
    ...
    def _concatenate_subtitles(self, videos, video_durations, output_name):
        ...
        try:
            ...
            for video in videos:
                if video.srt_path and os.path.exists(
                    os.path.join(settings.MEDIA_ROOT, "saved_srt", video.srt_path)
                ):
                    videos_with_subtitles.append((video, "srt_path"))
                    if video.raw_lang:
                        subtitle_languages.add(video.raw_lang)

                if video.translated_srt_path and os.path.exists(
                    os.path.join(
                        settings.MEDIA_ROOT, "saved_srt", video.translated_srt_path
                    )
                ):
            ...
            if len(subtitle_languages) > 1:
                print(
                    f"Warning: Multiple subtitle languages detected: {subtitle_languages}"
                )
                return
            ...
            for i, (video, subtitle_field) in enumerate(videos_with_subtitles):
                ...
                full_subtitle_path = os.path.join(
                    settings.MEDIA_ROOT, "subtitle", subtitle_path
                )
                ...
                try:
                    ...
                    adjusted_content = self._adjust_subtitle_timestamps(
                        subtitle_content, cumulative_time
                    )
            ...
            if concatenated_subtitles:
                ...
                output_subtitle_name = (
                    output_name.replace(".mp4", ".srt")
                    .replace(".mkv", ".srt")
                    .replace(".webm", ".srt")
                )
                output_subtitle_path = os.path.join(
                    settings.MEDIA_ROOT, "subtitle", output_subtitle_name
                )
                ...
                print(
                    f"Successfully created concatenated subtitle file: {output_subtitle_name}"
                )
    ...
    def _create_concatenated_video_record(
        self, original_videos, output_name, video_durations
    ):
        ...
        try:
            ...
            video_names = [
                v.name for v in original_videos[:3]

...
def _ensure_system_libstdcxx():
    """Prefer the system libstdc++ without re-execing the hosting process."""
...
def _bind_libs():
    ...
    if sys.platform == "win32":
        ...
        _ggml_lib = _load_shared(lib_dir / "libggml.dylib")
        _llama_lib = _load_shared(lib_dir / "libllama.dylib")
    else:
        for name in ("libggml-base.so", "libggml-cpu.so", "libggml-cuda.so"):
            path = lib_dir / name
            if path.exists():
                _load_shared(path)
        _ggml_lib = _load_shared(lib_dir / "libggml.so")
        _llama_lib = _load_shared(lib_dir / "libllama.so")

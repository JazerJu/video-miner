...
def get_model() -> WhisperModel:
    ...
    if _MODEL is None:
        ...
        # 3️⃣ 构建本地模型的完整路径，避免重新下载
        # faster-whisper 的 HuggingFace 缓存格式：models--Systran--faster-whisper-{model_name}
        local_model_path = os.path.join(model_dir, f"models--Systran--faster-whisper-{model_name}")

        # 检查本地模型是否存在
        if os.path.exists(local_model_path) and os.path.isfile(os.path.join(local_model_path, "model.bin")):
            print(f"Using local model: {local_model_path}")
            model_path = local_model_path
        else:
            # 如果本地不存在，使用模型名称（会触发下载）
            print(f"Local model not found, will download to: {model_dir}")
...
def get_multilingual_transcription_params():
    ...
    return {
        ...
        # 重复控制（修复重复输出问题）
        'repetition_penalty': 1.5,  # ✅ 提高惩罚，防止重复循环
        ...
        # 上下文
        'condition_on_previous_text': False,  # ✅ 关闭上下文记忆，防止重复扩散

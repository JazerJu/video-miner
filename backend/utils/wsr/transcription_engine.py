    ...
    def transcribe_audio(
        ...
        try:
            ...
            recognition = Recognition(
                model=transcription_settings.get(
...
class OpenAIWhisperEngine(TranscriptionEngine):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = transcription_settings.get(
...
class FasterWhisperEngine(TranscriptionEngine):
    """Faster-Whisper local transcription engine (optimized C++ implementation)"""
    ...
    def is_available(self) -> bool:
        try:
            from .fast_wsr import get_configured_model_name
            import os
            ...
            return os.path.exists(local_model_path) and os.path.isfile(
...
class RemoteVidGoEngine(TranscriptionEngine):
    ...
    def transcribe_audio(
        ...
        try:
                ...
                if status_response.status_code == 200:
                    ...
                    if status == "completed":
                        ...
                        if result_response.status_code == 200:
                            ...
                            raise Exception(
                                f"Failed to download transcription result: {result_response.text}"
                            )
                    ...
                    raise Exception(
                        f"Failed to check transcription status: {status_response.text}"
                    )
            ...
            progress_cb("Failed")
            raise Exception(
                f"Network error connecting to remote VidGo service: {str(e)}"
            )
        except Exception as e:
            progress_cb("Failed")
            raise
...
class TranscriptionEngineFactory:
    ...
    def get_engine_info(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "faster_whisper": {
                "name": "Faster-Whisper (Optimized Python Implementation)",
                "description": "GPU-accelerated transcription with word-level timestamps, supports CUDA",
                "type": "local",
                "languages": "Multi-language",
                "requires_api_key": False,
                "speed": "Very Fast (CUDA), Fast (CPU)",
                "quality": "High",
            },
            "whisper_cpp": {
        ...
        if fallback_engine and fallback_engine != engine_type:
            try:
                ...
                if fallback_engine_instance.is_available():
                    return fallback_engine_instance.transcribe_audio(
                        audio_file_path, progress_cb, language
                    )
                else:
                    raise Exception(

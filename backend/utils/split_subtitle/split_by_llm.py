...
def split_by_llm(
    ...
    prompt = VIDEO_SPLIT_PROMPT_TEMPLATE.format(sentence=text)
    ...
    try:
        ...
        if not result_cleaned.endswith("}"):
            logger.warning(
                f"[!] Response appears truncated - doesn't end with '}}'. Last 50 chars: {result_cleaned[-50:]}"
            )
            logger.warning(
                f"[!] This may indicate the response exceeded max_tokens limit"
            )
            ...
            if result_cleaned.endswith('"'):
                ...
                result_cleaned += '"\n}'
            logger.info(
                f"[DEBUG] Attempted to fix truncated JSON, new ending: {result_cleaned[-50:]}"
            )
        ...
        split_result = [
            segment.strip() for segment in split_text.split("<br>") if segment.strip()
        ]  # 将单个段落拆分为句子（各语言通用），通过strip去除文本两端的空格
        ...
        logger.error(
            f"[!] 原始内容预览 (前500字符): {result[:500] if result else 'None'}"
        )
        logger.error(
            f"[!] 原始内容结尾 (后200字符): {result[-200:] if result else 'None'}"
        )
        return []

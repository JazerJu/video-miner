...
if not logger.handlers:
    ...
    log_file = os.path.join(
        os.path.dirname(__file__), "..", "..", "logs", "translate.log"
    )
...
def step1_direct_translate_batch(
    batch_segments: ASRData,
    batch_start_idx: int,
    batch_size: int,
    all_segments: ASRData = None,
    use_cache: bool = True,
    source_lang: str = "en",
    target_lang: str = "zh",
    terms_to_note: str = "",
    ...
    for i, segment in enumerate(batch_segments, 1):  # i 从1开始，到len(batch_segments)
        input_json[str(i)] = {"original": segment.text}
    ...
    try:
        ...
        logger.debug(
            f"批次{batch_start_idx // batch_size + 1} 清理后的响应: {cleaned_response[:200]}..."
        )

        response_json = json.loads(cleaned_response)
        logger.debug(
            f"批次{batch_start_idx // batch_size + 1} JSON解析成功，包含{len(response_json)}个条目"
        )

        # 将LLM返回的结果（序号1-batch_size）映射回原始segments
        for batch_idx, segment in enumerate(
            batch_segments
        ):  # batch_idx: 0 to len(batch_segments)-1
            ...
            if llm_key in response_json and "direct" in response_json[llm_key]:
                ...
                logger.debug(
                    f"批次{batch_start_idx // batch_size + 1} 原始第{original_idx + 1}句(批内第{batch_idx + 1}句)直译完成: {segment.text[:30], segment.direct[:30]}..."
                )
            else:
                logger.warning(
                    f"批次{batch_start_idx // batch_size + 1} 缺少key '{llm_key}' 或 'direct' 字段"
                )
        ...
        logger.error(
            f"批次{batch_start_idx // batch_size + 1} LLM返回的JSON格式无效: {e}"
        )
        ...
        logger.error(
            f"批次{batch_start_idx // batch_size + 1} 处理响应时发生未知错误: {e}"
        )
        logger.error(f"错误类型: {type(e).__name__}")
        logger.error(f"原始响应: {repr(response)}")
        logger.error(
            f"清理后响应: {repr(cleaned_response) if 'cleaned_response' in locals() else 'N/A'}"
        )
...
def step1_direct_translate(
    ...
    logger.info(
        f"直译阶段：将{len(asr_data.segments)}个句子分为{len(batches)}个批次，每批最多{batch_size}句"
    )

    # 多线程处理批次
    def process_batch(batch_data):
        batch_segments, batch_start_idx = batch_data
        return step1_direct_translate_batch(
            batch_segments,
            batch_start_idx,
            batch_size,
            asr_data,
...
def step2_free_translate_batch(
    batch_segments: ASRData,
    batch_start_idx: int,
    batch_size: int,
    all_segments: ASRData = None,
    ...
    if all_segments and len(all_segments.segments) > 0:
        ...
        if prev_start_idx < prev_end_idx:
            ...
            previous_context = "\n".join(
                [f"{i + 1}. {seg.text}" for i, seg in enumerate(prev_segments)]
            )
        ...
        if next_start_idx < next_end_idx:
            next_segments = all_segments.segments[next_start_idx:next_end_idx]
            next_context = "\n".join(
                [f"{i + 1}. {seg.text}" for i, seg in enumerate(next_segments)]
            )

    # 准备输入JSON，序号从1开始到batch_size
    input_json = {}
    for i, segment in enumerate(batch_segments, 1):  # i 从1开始，到len(batch_segments)
        input_json[str(i)] = {"original": segment.text, "direct": segment.direct}
    ...
    try:
        ...
        for batch_idx, segment in enumerate(
            ...
            if llm_key in response_json:
                ...
                if "free" in response_json[llm_key]:
                    ...
                    logger.debug(
                        f"批次{batch_start_idx // batch_size + 1} 原始第{original_idx + 1}句(批内第{batch_idx + 1}句)翻译意见为：{segment.reflected[:30]}，意译完成: {segment.free[:30]}..."
                    )
            else:
                logger.warning(
                    f"批次{batch_start_idx // batch_size + 1} 缺少key '{llm_key}'"
                )
                logger.debug(f"响应中的keys: {list(response_json.keys())}")

    except json.JSONDecodeError as e:
        logger.error(
            f"批次{batch_start_idx // batch_size + 1} (意译阶段) LLM返回的JSON格式无效: {e}"
        )
        logger.error(f"JSON错误位置: 行{e.lineno}, 列{e.colno}")
        logger.error(f"错误消息: {e.msg}")
        logger.error(f"原始LLM响应长度: {len(response)} 字符")
        ...
        for batch_idx, segment in enumerate(batch_segments):
            if not hasattr(segment, "free") or not segment.free:
                segment.free = getattr(
                    segment, "direct", segment.text
                )  # 使用直译或原文作为默认意译
            if not hasattr(segment, "reflected") or not segment.reflected:
                segment.reflected = "翻译完成"
    except Exception as e:
        logger.error(
            f"批次{batch_start_idx // batch_size + 1} (意译阶段) 处理响应时发生未知错误: {e}"
        )
        logger.error(f"错误类型: {type(e).__name__}")
        logger.error(f"原始响应: {repr(response)}")
        logger.error(
...
def step2_free_translate(
    ...
    logger.info(
        f"意译阶段：将{len(asr_data.segments)}个句子分为{len(batches)}个批次，每批最多{batch_size}句"
    )

    # 多线程处理批次
    def process_batch(batch_data):
        batch_segments, batch_start_idx = batch_data
        return step2_free_translate_batch(
            batch_segments,
            batch_start_idx,
            batch_size,
            asr_data,
...
def two_step_translate(
    ...
    asr_data = step1_direct_translate(
        asr_data,
        use_cache,
        batch_size,
        num_threads,
